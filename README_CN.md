...
Sub2API 现在支持两种前端交付模式：

- `server.frontend_mode: embedded`（默认）
  - 使用发布二进制 / 镜像内置的前端
  - 适合标准安装和单文件部署
- `server.frontend_mode: external`
  - 从 `server.frontend_dir` 指定的目录读取 `index.html` 和静态资源
  - 适合不重编 Go 后端、直接替换或自定义前端
...
- `server.frontend_dir` 中必须存在 `index.html`
- 静态资源路径需要与前端构建产物保持一致（例如 `assets/...`）
- 外置模式下仍保留：公开配置注入、CSP nonce 注入、SPA fallback、ETag / 缓存逻辑
